...
<form action="./plugin.php?plugin=kl_data_call&action=setting&act=<?php echo $act; ?>" method="POST">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="table_b">
		...
		<tr><td align="left">个人定制：</td><td><input id="custom_tailor" name="custom_tailor" type="text" value="<?php echo isset($module['custom_tailor']) ? $module['custom_tailor'] : ''; ?>" /><font color="green"> * 此选项可以填入要调用的<strong>文章id</strong>,用<strong>半角的逗号(,)</strong>分隔即可，如(20,15,16,25)。</font><font color="red">(优先级：高)</font></td></tr>
		<tr><td align="left">所在分类：</td><td><select name="sort" id="sort"><option value="-1">所有分类</option><?php echo $sort_option_str ?></select></td></tr>
		<tr><td align="left">作者：</td><td><select id="author" name="author"><?php echo $author_option_str; ?></select></td></tr>
		<tr><td align="left">数据范围一(置顶)：</td><td><select id="filter" name="filter"><?php echo $filter_option_str; ?></select>　　　　　　<label><input id="nopwd" name="nopwd" type="checkbox" value="1" <?php if(isset($module['nopwd']) && $module['nopwd'] == 1) echo 'checked'; ?>/>不包含密码访问的文章</label></td></tr>
